...
'''
pykiwoom 오픈소스 참고: https://wikidocs.net/book/1173

키움 OpenAPI+ 자체가 업그레이드가 될 경우 대응
- 0. '자동로그인'을 해제
- 1. OpenAPI 탑재 프로그램 실행시
- 2. 키움 OpenAPI+ 업그레이드 팝업이 발생
- 3. 업그레이드 팝업은 건드리지 않고, 탑재 프로그램을 종료시켜야 함
- 4. 다음으로 업그레이드 팝업을 확인시켜 OpenAPI+ 업그레이드 실행
- 5. 업그레이드 후 확인 팝업차이 생성되는데, 이를 확인하고 종료
- 6. '자동로그인' 다시 설정

이 과정이 복잡하기 때문에 기 구현된 내용을 그대로 활용 
'''
...
if __name__ == "__main__":
    ...
    version(user_id=config_db['id'], user_pass=config_db['password'], user_cert=config_db['cert_password'])